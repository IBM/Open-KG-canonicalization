...
def macroPrecision(C_clust2ele, E_ele2clust):
	...
	for clust_id, cluster in C_clust2ele.items():
		isFirst = True
		res = set()
		for ele in cluster:
		...
		if   len(res) == 1: num_prec += 1
			...
			raise ValueError('ERROR In Clustering with cluster ID: {0}'.format(res))
...
def get_metrics(C_ele2clust, C_clust2ele, E_ele2clust, E_clust2ent):
	macro_prec 	= macroPrecision(C_clust2ele, E_ele2clust)
	macro_recall 	= macroPrecision(E_clust2ent, C_ele2clust)
	macro_f1	= calcF1(macro_prec, macro_recall)
